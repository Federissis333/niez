document.addEventListener("DOMContentLoaded", () => {
    ...
    fetchData({
      uds: ["933428472452354108", "388349345386201099"],
    });
  });
  ...
  function fetchUser(uid, index) {
    fetch(`https://api.kiritoapi.shop/api/v4/discord/user/${uid}`)
      .then((response) => {
        if (!response.ok) {
  ...
  function atualizarprofile(index, userData) {
   ...
   imgElement.src = userData.user.avatarUrl; // Corrigido aqui
   bioElement.textContent = userData.user_profile.bio || " ";
